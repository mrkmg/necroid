# Necroid Architecture

Reference doc for Necroid's design — install model, layouts, manifest schema, reconciliation, build constraints, directory roles. CLAUDE.md covers the two day-to-day flows (working on the tool, working on mods); this doc is the deep context behind them. Read it before touching install/uninstall/resync internals or anything that writes to the PZ install.
...
**Distribution model:** the repo is git-tracked for sharing with other modders, but nothing PZ-owned ships through git — and the workspace itself doesn't even live in the checkout (it's inside the PZ install). `.gitignore` excludes `data/.necroid-pointer.json`, `data/.mod-enter.json`, `data/.update-cache.json`, `data/tools/*` (auto-fetched JDK/git/vineflower), `dist/`, `build/`, Python caches, plus the legacy paths from pre-PZ-anchored Necroid (kept ignored in case anyone migrates a checkout that still has them). `src-*/` (per-mod editable working trees) is **not** gitignored — IDE language servers (Pylance, etc.) skip gitignored paths, which would break code intel. Commits are blocked at `.githooks/pre-commit` instead, which `necroid init` installs and wires up via `git config core.hooksPath .githooks`. On a fresh clone without `init`, run that one command yourself to enable the guard (or pass `--no-verify` once to bypass for a deliberate `src-*/` snapshot). On a fresh clone, `necroid init` writes the pointer + bootstraps `<pz>/necroid/` from the user's own Steam install — they must own a copy of PZ. Only `necroid/` (Python source), `packaging/`, `assets/`, `.githooks/`, `mods/` (the patch-set library at the repo root), and docs are tracked. Releases ship via GitHub Releases at `github.com/mrkmg/necroid` — tag, run `packaging/build_dist.py`, zip `dist/`, attach to the tagged release. **3rd-party mod authors use the same layout**: drop `necroid` at the root of their own repo, `necroid init` (which scaffolds `mods/`, writes a default `.gitignore`, and installs the `src-*/` pre-commit guard), develop under `mods/`, commit `README.md` + `mods/`. Users then `necroid import owner/repo` and the canonical `mods/<name>/mod.json` layout is picked up automatically.
...
- `src-<modname>/{zombie,astar,com,de,fmod,javax,org,se}/` — per-mod editable working tree at the **repo root**. `enter` seeds it from pristine + patches (preserving contents if it already exists), `capture` reads it back into the mod's patch set, `reset` re-seeds it, `clean` deletes it. **Not** gitignored (IDE language servers need it visible); commits are blocked by `.githooks/pre-commit`.
